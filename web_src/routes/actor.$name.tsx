...
const ActressNameComponent: React.FC = () => {
  ...
  useTitle(`演员：${name as string}`);
  const { data: videos }: ResWithActressName = useLoaderData({ from: "/actor/$name" });
...
export const Route = createFileRoute("/actor/$name")({
  ...
  loader: async ({ params: { name } }) => (await fetcher(`/api/query/actor/${name}`)),
